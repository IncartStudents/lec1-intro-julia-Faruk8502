...
#3 - list comprehension
   result_lc = [double(x) for x in arr] 
#= Чем эти подходы лучше явного цикле:
      - Они делают код локаничнее
      - Выше производительность
=#
# Перемножить вектор-строку [1 2 3] на вектор-столбец [10,20,30] и объяснить результат
#=
  Произойдёт матричное умножение (каждый элемент первого вектора умножится на 
соответствующий элемент второго вектора и результаты сложаться) и на выходе мы получим 
одно число
=#

# В одну строку выбрать из массива [1, -2, 2, 3, 4, -5, 0] только четные и положительные числа
 
    filter(x -> x > 0 && x % 2 == 0, [1, -2, 2, 3, 4, -5, 0]
...
#=
  number в данном случае получает первое число из ряда тех, которых нет в векторе 
  отсортированных уникальных чисел соответствующих названиям файов, содержащих 
  =#
# Упростить этот код обработки:
    using Random
    Random.seed!(123)
    
    #  Генерацию имён оставляем как есть
    names = [rand('A':'Z') * '_' * rand('0':'9') * rand([".csv", ".bin"]) for _ in 1:100]
    
    # Однострочное решение:
    number = setdiff(0:9, parse.(Int, last.(split.(first.(split.
                         (names[startswith.(names, "A")], "."), "_"))))[1]
...
=#
    struct LazySquaredArray <: AbstractVector{Int}
        len::Int
    end
    
    Base.size(A::LazySquaredArray) = (A.len,)
    
    Base.getindex(A::LazySquaredArray, i::Int) = (i - 1)^2
    
    Base.getindex(A::LazySquaredArray, I::AbstractVector) = [(i-1)^2 for i in I]
    
    Base.show(io::IO, A::LazySquaredArray) = print(io, "LazySquaredArray($(A.len))")
#=
